...
Tuna::Tuna(int id, Cave *home, Model *world) : Fish('T', id, home, 10, world)
{ // for sharks
	this->home = home;
	this->state = 'h';
	this->energy = 15;
...
bool Tuna::start_mating(Tuna *fish_mate)
{
	int tick_count = time;
	mate = fish_mate;
	if ((Fish::get_size() >= 40 && Fish::get_size() <= 60) && (fish_mate->Fish::get_size() >= 40 && fish_mate->Fish::get_size() <= 60) && (state == 'h') && (fish_mate->state == 'h'))
